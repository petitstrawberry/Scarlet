...
#[test_case]
fn test_load_elf() {
    use crate::fs::{FileObject, VfsManager};
    ...
    let file = manager.open(file_path, 0).expect("Failed to open file");
    ...
    let entry_point = load_elf_into_task(file.as_ref(), &mut task).expect("Failed to load ELF file");
...
fn test_load_elf_invalid_magic() {
    ...
    let file = manager.open("/invalid.elf", 0).unwrap();
    ...
    let result = load_elf_into_task(file.as_ref(), &mut task);
...
fn test_load_elf_invalid_alignment() {
    ...
    let mut file = manager.open("/invalid_align.elf", 0o777).map_err(|_| "Failed to create file").unwrap();
    file.write(&invalid_elf_data).expect("Failed to write invalid ELF data");
...
fn test_load_elf_bss_zeroed() {
    ...
    manager.create_regular_file(file_path).expect("Failed to create test file");
    let mut file = manager.open(file_path, 0o777).map_err(|_| "Failed to create file").unwrap();
    ...
    // Load the ELF file into the task
    load_elf_into_task(file.as_ref(), &mut task).expect("Failed to load ELF file");
