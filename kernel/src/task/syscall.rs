...
use crate::task::{CloneFlags, CloneFlagsDef, WaitError, get_parent_waitpid_waker, get_waitpid_waker};
        ...
        Ok(mut child_task) => {
            ...
            if clone_flags.is_set(CloneFlagsDef::SetTls) {
                ...
                if let Some(abi) = child_task.default_abi.as_mut() {
                    // Try to downcast to ScarletAbi
                    use crate::abi::scarlet::riscv64::ScarletAbi;
                    if let Some(scarlet_abi) = abi.as_any_mut().downcast_mut::<ScarletAbi>() {
                        scarlet_abi.set_tls_pointer(tls_ptr);
                    }
                }

                // Set tp register (x4) for RISC-V TLS
...
pub fn sys_get_tls(trapframe: &mut Trapframe) -> usize {
    ...
    let tls_ptr = if let Some(abi) = task.default_abi.as_ref() {
        use crate::abi::scarlet::riscv64::ScarletAbi;
        if let Some(scarlet_abi) = abi.as_any().downcast_ref::<ScarletAbi>() {
