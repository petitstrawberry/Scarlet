...
pub fn sys_mmap(abi: &mut LinuxAarch64Abi, trapframe: &mut Trapframe) -> usize {
...
pub fn sys_mprotect(_abi: &mut LinuxAarch64Abi, trapframe: &mut Trapframe) -> usize {
    // Linux protection flags
    const PROT_READ: usize = 0x1;
    const PROT_WRITE: usize = 0x2;
    const PROT_EXEC: usize = 0x4;
...
pub fn sys_munmap(_abi: &mut LinuxAarch64Abi, trapframe: &mut Trapframe) -> usize {
    let task = match mytask() {
        Some(task) => task,
        None => return usize::MAX,
    };
