use crate::{
    abi::octox::riscv64::fs::octoxfs::{Dirent, Stat},
    arch::Trapframe,
...
/// Convert Scarlet DirectoryEntry to octox Dirent and write to buffer
fn read_directory_as_octox_dirent(buf_ptr: *mut u8, count: usize, buffer_data: &[u8]) -> usize {
...
pub fn sys_exec(
    _abi: &mut crate::abi::octox::riscv64::OctoxRiscv64Abi,
...
pub fn sys_open(
    abi: &mut crate::abi::octox::riscv64::OctoxRiscv64Abi,
    trapframe: &mut Trapframe,
) -> usize {
    let task = mytask().unwrap();
    let path_ptr = task
...
/// sys_dup2 - duplicate file descriptor to a specific target fd
/// octox extension over xv6
pub fn sys_dup2(
    abi: &mut crate::abi::octox::riscv64::OctoxRiscv64Abi,
    trapframe: &mut Trapframe,
) -> usize {
    let task = mytask().unwrap();
    let src_fd = trapframe.get_arg(0) as usize;
    let dst_fd = trapframe.get_arg(1) as usize;
    trapframe.increment_pc_next(task);

    // If src_fd == dst_fd, just return dst_fd
    if src_fd == dst_fd {
        // Verify src_fd is valid
        if abi.get_handle(src_fd).is_some() {
            return dst_fd;
        } else {
            return usize::MAX; // Invalid source fd
        }
    }

    // Get handle from source fd
    let src_handle = match abi.get_handle(src_fd) {
        Some(h) => h,
        None => return usize::MAX, // Invalid source fd
    };

    // Get the kernel object to clone
    let kernel_obj = match task.handle_table.get(src_handle) {
        Some(obj) => obj.clone(),
        None => return usize::MAX, // Invalid handle
    };

    // Close dst_fd if it's open
    if let Some(dst_handle) = abi.remove_fd(dst_fd) {
        task.handle_table.remove(dst_handle);
    }

    // Insert the cloned object
    let new_handle = match task.handle_table.insert(kernel_obj) {
        Ok(h) => h,
        Err(_) => return usize::MAX, // Handle table full
    };

    // Map the new handle to dst_fd
    abi.fd_to_handle.insert(dst_fd, new_handle as u32);
    // Remove dst_fd from free list if present
    abi.free_fds.retain(|&fd| fd != dst_fd);
...
pub fn sys_close(
    ...
    // Get handle from octox fd and remove mapping
        ...
        match stream.read(&mut temp_buffer) {
            Ok(n) => {
                ...
                if n > 0 && n >= directory_entry_size {
                    // Convert DirectoryEntry to octox Dirent
                    let converted_bytes =
                        read_directory_as_octox_dirent(buf_ptr, count, &temp_buffer[..n]);
                    if converted_bytes > 0 {
                        return converted_bytes; // Return converted octox dirent size
                    }
                }
...
pub fn sys_link(
    ...
    match vfs.create_hardlink(&src_path, &dst_path) {
        ...
        Err(err) => {
            ...
            // Map VFS errors to appropriate errno values for octox
            match err.kind {
...
pub fn sys_fcntl(
    ...
    if task.handle_table.get(handle).is_none() {
        return usize::MAX; // Invalid handle
    }

    // Handle fcntl commands
    // octox FcntlCmd values:
    // F_DUPFD = 0
    // F_GETFD = 1
    // F_SETFD = 2
    // F_GETFL = 3
    ...
    match cmd {
        0 => {
            ...
            // For simplicity, just duplicate to lowest available
            if let Some(kernel_obj) = task.handle_table.get(handle) {
                let cloned = kernel_obj.clone();
                match task.handle_table.insert(cloned) {
                    Ok(new_handle) => match abi.allocate_fd(new_handle as u32) {
                        Ok(new_fd) => new_fd,
                        Err(_) => usize::MAX,
                    },
                    Err(_) => usize::MAX,
        ...
        1 => {
            ...
            // Return close-on-exec flag (simplified: always 0)
            0
        }
        2 => {
            // F_SETFD - set file descriptor flags
            // Accept but ignore (simplified implementation)
            0
        }
        3 => {
            // F_GETFL - get file status flags
            // Return read/write mode (simplified: O_RDWR = 2)
            2
        }
        4 => {
        ...
        7 => {
            // F_SETLKW - set file lock and wait
            // Accept but ignore (simplified implementation - no locking)
            0
        }
        _ => {
            // Unknown command
