...
pub fn sys_kill(
    ...
    // For octox compatibility, only signal 9 (SIGKILL) is implemented for now
    // octox kill() just takes pid, no signal argument
    if pid == 0 {
        return usize::MAX; // -1 (cannot kill pid 0)
    }

    // Find the target task via scheduler
    let scheduler = get_scheduler();
    if let Some(target_task) = scheduler.get_task_by_id(pid) {
        // For octox compatibility, immediately terminate the target task
...
pub fn sys_uptime(
    ...
    // Use the timer tick counter from the kernel
    crate::timer::get_tick() as usize
}
