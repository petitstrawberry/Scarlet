...
use super::super::{
    AccessMode, HandleMetadata, HandleTable, HandleType, KernelObject, SpecialSemantics,
    StandardInputOutput,
};
...
fn test_special_semantics() {
    ...
    assert_eq!(
        close_on_exec_metadata.special_semantics.unwrap(),
        SpecialSemantics::CloseOnExec
    );
}
...
fn test_metadata_inference_for_files() {
    ...
    let file_obj = Arc::new(MockFileObject::with_name_and_content(
        "test.txt",
        "Hello, World!",
    ));
...
fn test_explicit_metadata_insertion() {
    ...
    let handle = table
        .insert_with_metadata(kernel_obj, stdin_metadata.clone())
    ...
    assert_eq!(
        retrieved_metadata.special_semantics,
        stdin_metadata.special_semantics
    );
}
...
fn test_metadata_persistence_across_operations() {
    ...
    let handle = table
        .insert_with_metadata(kernel_obj, metadata.clone())
...
fn test_metadata_with_iterator() {
    ...
    let _h1 = table
        .insert_with_metadata(KernelObject::File(file1), regular_metadata)
        .unwrap();
    let _h2 = table
        .insert_with_metadata(KernelObject::File(file2), stdin_metadata)
        .unwrap();
    let _h3 = table
        .insert_with_metadata(KernelObject::Pipe(pipe1), ipc_metadata)
        .unwrap();
