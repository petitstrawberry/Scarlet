...
//!
//! Apple Silicon PCIe root complex driver for T8103 (M1) and later.
//! Reference: asahi-linux `drivers/pci/controller/pcie-apple.c`
//!
//! # Architecture
//!
//! Apple PCIe is ECAM-compliant for config space access, but uses a custom
//! port-based initialization sequence with PHY setup, reference clock
//! configuration, and link training.
//!
//! # Register Blocks (per port)
//!
//! - RC registers: core root complex (shared across ports)
//! - PHY registers: per-port PHY at `0x84000 + 0x4000 * port_idx`
//! - Port registers: per-port at offsets defined in device tree `reg` property
...
use crate::drivers::pic::apple_msi::MsiPortConfig;
use crate::{
    arch::mmio,
    device::{
        DeviceInfo,
...
// =============================================================================
// Core RC Register Offsets (shared)
// =============================================================================
...
// =============================================================================
// Port Register Offsets (per port, relative to port base)
// =============================================================================
...
impl ApplePciePort {
    fn read32(&self, offset: usize) -> u32 {
        // SAFETY: offset is within the MMIO-mapped port region
    ...
    fn phy_read32(&self, offset: usize) -> u32 {
        // SAFETY: offset is within the MMIO-mapped PHY region
    ...
    fn ecam_read16(&self, bus: u32, dev: u32, func: u32, offset: u16) -> u16 {
        let addr = self.ecam_base
            ...
            + (offset as usize);
        // SAFETY: addr is within the ECAM region
    ...
    /// Read vendor ID from ECAM config space for bus 0, dev 0, func 0
    pub fn read_root_vendor_id(&self) -> u16 {
        self.ecam_read16(0, 0, 0, PCI_CONFIG_VENDOR_ID)
    }

    /// Enable bus master and memory space on root port
...
fn probe_fn(device: &PlatformDeviceInfo) -> Result<(), &'static str> {
    ...
    if let Some(iommu_prop) = device.property("iommu-map") {
        ...
        while offset + entry_size <= bytes.len() {
            ...
            if let Some(dart) = crate::drivers::iommu::apple_dart::get_dart_by_phandle(dart_phandle)
            {
                early_println!(
    ...
    if let Some(msi_prop) = device.property("msi-ranges") {
        ...
        // Format: <phandle(4) type(4) number(4) flags(4) nvecs(4)>
        // phandle is at index 0, AIC #interrupt-cells=3, so nvecs at index 4
        if bytes.len() >= 20 {
    ...
    for (port_idx, port_mem) in port_regions.iter().enumerate() {
        ...
        let port_vaddr =
            crate::vm::ioremap(port_paddr, port_size).map_err(|_| "pcie: port ioremap failed")?;
...
driver_initcall!(register_pcie_driver);
