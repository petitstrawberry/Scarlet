...
mod integration_tests {
    ...
    fn test_char_device_id_assignment() {
        ...
        assert!(
            fb.created_char_device_id.read().is_some(),
            "Character device should be automatically created"
        );
    ...
    fn test_framebuffer_boundary_conditions() {
        ...
        device.set_framebuffer_config(config.clone());
        let fb_addr = crate::mem::page::allocate_contiguous_raw_pages(1) as usize; // One page
        device.set_framebuffer_address(fb_addr);
        let shared_device: Arc<dyn Device> = Arc::new(device);
        ...
        assert!(
            char_device.write_at(8, &[0x00]).is_err()
                || char_device.write_at(8, &[0x00]).unwrap() == 0
        );
    ...
    fn test_devfs_integration_with_device_manager() {
        ...
        for (i, device_name) in device_names.iter().enumerate() {
            ...
            // Register device with DeviceManager first
            let device_id = device_manager
                .register_device_with_name(device_name.to_string(), shared_device.clone());
    ...
    fn test_dev_fb0_gradient_drawing() {
        ...
        let read_count = fb_char_device
            .read_at(bottom_right_pos as u64, &mut bottom_right_pixel)
            .unwrap();
        assert_eq!(read_count, 4);
        ...
        let middle_pos = (width / 2 * bytes_per_pixel as u32
            + (height / 2) * width * bytes_per_pixel as u32) as usize;

        let mut middle_pixel = [0u8; 4];
        let read_count = fb_char_device
            .read_at(middle_pos as u64, &mut middle_pixel)
            .unwrap();
        assert_eq!(read_count, 4);
