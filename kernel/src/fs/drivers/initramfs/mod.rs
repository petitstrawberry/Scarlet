...
pub struct Initramfs {
    id: usize,
    name: &'static str,
    entries: Mutex<Vec<InitramfsEntry>>, // List of entries
    mounted: bool,
    mount_point: String,
}
...
impl Initramfs {
    ...
    fn parse_cpio(cpio_data: &[u8]) -> Result<Vec<InitramfsEntry>> {
...
impl FileOperations for Initramfs {
    fn open(&self, path: &str, _flags: u32) -> Result<Box<dyn crate::fs::FileHandle>> {
        ...
        if let Some(entry) = entries.iter().find(|e| e.name == path) {
            Ok(Box::new(InitramfsFileHandle {
...
impl crate::fs::FileHandle for InitramfsFileHandle {
