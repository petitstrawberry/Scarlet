...
mod tests {
    ...
    fn test_cpiofs_dir_symlink_basic() {
        ...
        assert!(matches!(
            dir_symlink_node.file_type().unwrap(),
            FileType::SymbolicLink(_)
        ));
    ...
    fn test_cpiofs_read_dir_link_error() {
        ...
        let file_node = cpiofs
            .lookup(&testdir_node, &"file.txt".to_string())
            .unwrap();
    ...
    fn test_vfs_directory_symlink_resolution() {
        ...
        assert!(
            result.is_ok(),
            "Should be able to resolve file through directory symlink"
        );
        ...
        assert!(
            result.is_ok(),
            "Should be able to resolve directory symlink"
        );
    ...
    fn test_vfs_readdir_through_symlink() {
        ...
        assert!(
            result.is_ok(),
            "Should be able to list directory through symlink"
        );
        ...
        let symlink_files: BTreeSet<_> = entries
            .iter()
            .filter(|e| e.name != "." && e.name != "..")
            .map(|e| &e.name)
            .collect();
        let direct_files: BTreeSet<_> = direct_entries
            .iter()
            .filter(|e| e.name != "." && e.name != "..")
            .map(|e| &e.name)
            .collect();
        ...
        assert_eq!(
            symlink_files, direct_files,
            "Directory listings should be equivalent"
        );
    }
    ...
    fn test_vfs_path_resolution_no_follow() {
        use crate::fs::vfs_v2::manager::{PathResolutionOptions, VfsManager};
        ...
        let result =
            vfs.resolve_path_with_options("/link.txt", &PathResolutionOptions::no_follow());
        assert!(
            result.is_ok(),
            "Should be able to resolve symlink with no_follow"
        );
        let (entry, _mount_point) = result.unwrap();
        // Should return the symlink itself, not the target
        assert!(matches!(
            entry.node().file_type().unwrap(),
            FileType::SymbolicLink(_)
        ));
    ...
    fn test_vfs_path_resolution_no_follow() {
        ...
        assert!(
            result.is_ok(),
            "Should be able to resolve directory symlink with no_follow"
        );
        let (entry, _mount_point) = result.unwrap();
        ...
        assert!(
            result.is_ok(),
            "Should be able to resolve directory symlink normally"
        );
        let (entry, _mount_point) = result.unwrap();
