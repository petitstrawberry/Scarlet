...
impl FileSystemOperations for DevFS {
    ...
    fn lookup(
        ...
        let dev_node = Arc::downcast::<DevNode>(parent.clone()).map_err(|_| {
            FileSystemError::new(
    ...
    fn readdir(
        ...
        let dev_node = Arc::downcast::<DevNode>(node.clone()).map_err(|_| {
            FileSystemError::new(
                FileSystemErrorKind::NotSupported,
                "Invalid node type for DevFS",
            )
...
mod tests {
    ...
    fn test_devfs_device_discovery() {
        ...
        let _device_id =
            device_manager.register_device_with_name("test_devfs_device".to_string(), test_device);
        ...
        assert!(
            ...
            "DevFS readdir should work without error"
        );
    }
    ...
    fn test_devfs_with_real_devices() {
        ...
        let _char_device_id =
            device_manager.register_device_with_name("tty0".to_string(), char_device.clone());

        // Register a block device (disk-like)
        let block_device = Arc::new(MockBlockDevice::new("sda", 512, 1000));
        let _block_device_id =
            device_manager.register_device_with_name("sda".to_string(), block_device.clone());
    ...
    fn test_devfs_driver_registration() {
        ...
        assert!(
            fs_driver_manager.has_driver("devfs"),
            "DevFS driver should be registered"
        );
        ...
        assert!(
            devfs_result.is_ok(),
            "Should be able to create DevFS through driver manager"
        );
    ...
    fn test_devfs_device_file_operations() {
        ...
        let _device_id = device_manager
            .register_device_with_name("test_char_dev".to_string(), char_device.clone());
        ...
        assert!(
            device_node_result.is_ok(),
            "Should be able to lookup character device"
        );
        ...
        assert!(
            file_result.is_ok(),
            "Should be able to open character device file"
        );
        ...
        assert!(
            ...
            "Should be able to get device file metadata"
        );
    ...
    fn test_devfs_directory_operations() {
        ...
        let _device_id = device_manager
            .register_device_with_name("test_dir_ops".to_string(), char_device.clone());
        ...
        assert!(
            dir_file_result.is_ok(),
            "Should be able to open directory in devfs"
        );
        ...
        assert!(
            dir_entry.is_some(),
            "Should be able to parse directory entry"
        );
        ...
        assert!(
            name == "." || name == ".." || name == "test_dir_ops",
            "Entry name should be '.', '..' or 'test_dir_ops', got: {}",
            name
        );
