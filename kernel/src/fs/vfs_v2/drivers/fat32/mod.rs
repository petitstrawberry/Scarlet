...
pub use node::{Fat32DirectoryObject, Fat32FileObject, Fat32Node};
pub use structures::*;
...
impl Fat32FileSystem {
        ...
        loop {
            ...
            for i in 0..entries_per_cluster {
                ...
                if dir_entry.is_long_filename() {
                    // Cast to LFN entry
                    let lfn_entry =
                        unsafe { &*(entry_bytes.as_ptr() as *const structures::Fat32LFNEntry) };
        ...
        for i in 0..self.sectors_per_cluster {
            ...
            if let Some(result) = results.first() {
                match &result.result {
                    Ok(_) => {
                        ...
                        if end_offset <= buffer.len()
                            && result.request.buffer.len() >= self.bytes_per_sector as usize
                        {
                            buffer[start_offset..end_offset].copy_from_slice(
                                &result.request.buffer[..self.bytes_per_sector as usize],
                            );
                        }
                    }
                    Err(e) => {
                ...
                return Err(FileSystemError::new(
                    FileSystemErrorKind::IoError,
                    format!(
                        "No result from block device for cluster {} sector {}",
                        cluster,
                        cluster_sector + i
                    ),
                ));
    ...
    fn read_fat_entry(&self, cluster: u32) -> Result<u32, FileSystemError> {
        ...
        let fat_sector =
            self.boot_sector.reserved_sectors as u32 + (fat_offset / self.bytes_per_sector);
    ...
    fn write_fat_entry(&self, cluster: u32, value: u32) -> Result<(), FileSystemError> {
        ...
        for fat_copy in 0..self.boot_sector.fat_count {
            let current_fat_sector =
                fat_sector + (fat_copy as u32 * self.boot_sector.sectors_per_fat);
    ...
    fn initialize_directory(
        ...
        let actual_parent = if parent_cluster == self.root_cluster {
            0
        } else {
            parent_cluster
        };
    ...
    fn update_fs_info_allocated_cluster(
        &self,
                        ...
                        if &buffer[0..4] == b"RRaA" && &buffer[484..488] == b"rrAa" {
                            ...
                            let write_request =
                                Box::new(crate::device::block::request::BlockIORequest {
                                    request_type:
                                        crate::device::block::request::BlockIORequestType::Write,
                                    sector: fs_info_lba,
                                    sector_count: 1,
                                    head: 0,
                                    cylinder: 0,
    ...
    fn write_cluster_data(&self, cluster: u32, data: &[u8]) -> Result<(), FileSystemError> {
        ...
        for sector_offset in 0..self.sectors_per_cluster {
            ...
            let sector_data =
                buffer[buffer_offset..buffer_offset + self.bytes_per_sector as usize].to_vec();
        ...
        for (i, result) in results.iter().enumerate() {
            if result.result.is_err() {
                return Err(FileSystemError::new(
                    FileSystemErrorKind::IoError,
                    format!(
                        "Failed to write cluster {} sector {}",
                        cluster,
                        first_sector_of_cluster + i as u32
                    ),
                ));
            }
        }
    ...
    fn read_directory_entries(
        ...
        for chunk in data.chunks(entry_size) {
            ...
            let cluster = ((chunk[21] as u32) << 24)
                | ((chunk[20] as u32) << 16)
                | ((chunk[27] as u32) << 8)
                | (chunk[26] as u32);
    ...
    fn write_directory_entry_with_name(
        &self,
        dir_cluster: u32,
        filename: &str,
        cluster: u32,
            ...
            if next_cluster >= 0x0FFFFFF8 {
                ...
                for (j, entry_to_write) in entries_to_write.iter().enumerate() {
                    ...
                    match entry_to_write {
                        EntryToWrite::LFN(lfn_entry) => {
                            let entry_bytes = unsafe {
                                core::slice::from_raw_parts(lfn_entry as *const _ as *const u8, 32)
                            };
                        ...
                        EntryToWrite::SFN(sfn_entry) => {
                            let entry_bytes = unsafe {
                                core::slice::from_raw_parts(sfn_entry as *const _ as *const u8, 32)
                            };
                            empty_cluster[offset..offset + 32].copy_from_slice(entry_bytes);
            ...
            while i < entries_per_cluster {
                ...
                let dir_entry = unsafe {
                    core::ptr::read_unaligned(
                        entry_bytes.as_ptr() as *const structures::Fat32DirectoryEntry
                    )
                };
                    ...
                    for j in lfn_start..i {
                        ...
                        if lfn_bytes[11] == 0x0F {
                            let lfn_entry = unsafe {
                                core::ptr::read_unaligned(
                                    lfn_bytes.as_ptr() as *const structures::Fat32LFNEntry
                                )
                            };
    ...
    fn generate_sfn(filename: &str, numeric_tail: Option<u32>) -> [u8; 11] {
        ...
        let (name_part, ext_part) = if let Some(dot_pos) = clean_filename.rfind('.') {
            (
                &clean_filename[..dot_pos],
                Some(&clean_filename[dot_pos + 1..]),
            )
        } else {
...
impl FileSystemOperations for Fat32FileSystem {
    ...
    fn lookup(
        ...
        let fat32_parent = parent.as_any().downcast_ref::<Fat32Node>().ok_or_else(|| {
            FileSystemError::new(
    ...
    fn open(
        ...
        let fat32_node = node.as_any().downcast_ref::<Fat32Node>().ok_or_else(|| {
            FileSystemError::new(
                FileSystemErrorKind::NotSupported,
                "Invalid node type for FAT32",
            )
        ...
        match fat32_node.file_type() {
            Ok(FileType::RegularFile) => {
                ...
                Ok(Arc::new(Fat32FileObject::new(
                    Arc::new(fat32_node.clone()),
                    parent_cluster,
                )))
            }
            Ok(FileType::Directory) => Ok(Arc::new(Fat32DirectoryObject::new(Arc::new(
                fat32_node.clone(),
            )))),
    ...
    fn create(
        ...
        let new_node = match file_type {
            ...
            FileType::Directory => {
                ...
                let dir_cluster = self.allocate_cluster().map_err(|e| {
                    FileSystemError::new(
                        FileSystemErrorKind::NoSpace,
