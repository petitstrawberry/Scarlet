...
fn test_fat32_mockdevice_cluster_io() {
    ...
    fat32_fs
        .write_cluster_data(test_cluster, test_data)
        .expect("Failed to write cluster");

    // Read back the data
    let read_data = fat32_fs
        .read_cluster(test_cluster)
...
fn test_fat32_mockdevice_fat_operations() {
    ...
    // Write FAT entry
    fat32_fs
        .write_fat_entry(test_cluster, test_value)
        .expect("Failed to write FAT entry");

    // Read back FAT entry
    let read_value = fat32_fs
        .read_fat_entry(test_cluster)
        .expect("Failed to read FAT entry");
    ...
    fat32_fs
        .write_fat_entry(cluster1, cluster2)
        .expect("Failed to write FAT entry");
    fat32_fs
        .write_fat_entry(cluster2, cluster3)
        .expect("Failed to write FAT entry");
    fat32_fs
        .write_fat_entry(cluster3, 0x0FFFFFF8)
        .expect("Failed to write FAT entry");

    // Verify chain
    assert_eq!(
        fat32_fs
            .read_fat_entry(cluster1)
            .expect("Failed to read FAT entry"),
        cluster2
    );
    assert_eq!(
        fat32_fs
            .read_fat_entry(cluster2)
            .expect("Failed to read FAT entry"),
        cluster3
    );
    assert_eq!(
        fat32_fs
            .read_fat_entry(cluster3)
            .expect("Failed to read FAT entry"),
        0x0FFFFFF8
    );
}
...
fn test_fat32_mockdevice_file_content_io() {
    ...
    // Write file content
    let actual_cluster = fat32_fs
        .write_file_content(start_cluster, small_data)
        .expect("Failed to write file content");

    // Read back file content
    let read_content = fat32_fs
        .read_file_content(actual_cluster, small_data.len())
    ...
    for i in 0..100 {
        ...
        large_data.extend_from_slice(
            format!("Line {}: This is test data for multi-cluster file.\n", i).as_bytes(),
        );
    }
    ...
    let actual_large_cluster = fat32_fs
        .write_file_content(large_start_cluster, &large_data)
        .expect("Failed to write large file content");

    // Read back large file content
    let read_large_content = fat32_fs
        .read_file_content(actual_large_cluster, large_data.len())
        .expect("Failed to read large file content");
...
fn test_fat32_mockdevice_partial_read() {
    ...
    let actual_cluster = fat32_fs
        .write_file_content(start_cluster, test_data)
        .expect("Failed to write file content");

    // Test partial reads of different sizes
    let partial_size = 20;
    let partial_content = fat32_fs
        .read_file_content(actual_cluster, partial_size)
        .expect("Failed to read partial content");
    ...
    let exact_content = fat32_fs
        .read_file_content(actual_cluster, test_data.len())
        .expect("Failed to read exact content");
    ...
    let cluster_size = (fat32_fs.sectors_per_cluster * fat32_fs.bytes_per_sector) as usize;
    let full_cluster = fat32_fs
        .read_cluster(actual_cluster)
        .expect("Failed to read full cluster");
    ...
    for i in test_data.len()..cluster_size {
        assert_eq!(
            full_cluster[i], 0,
            "Expected zero at position {} but found {}",
            i, full_cluster[i]
        );
    }
}
...
fn create_test_fat32_device() -> MockBlockDevice {
    ...
    for fat_copy in 0..boot_sector.fat_count {
        for sector_offset in 0..boot_sector.sectors_per_fat {
            let fat_sector_addr = fat_start_sector
                + (fat_copy as usize * boot_sector.sectors_per_fat as usize)
...
fn test_fat32_virtio_blk_file_operations() {
    ...
    match fs_driver_manager.create_from_block("fat32", block_device_arc, 512) {
        ...
        Err(e) => {
            panic!(
                "Failed to create FAT32 filesystem from virtio-blk device: {:?}",
                e
            );
        }
    }
    ...
    match fs_driver_manager.create_from_block("fat32", Arc::new(virtio_dev), 512) {
        Ok(fs) => {
            ...
            match fs.lookup(&root_node, &String::from("test_write.txt")) {
                        ...
                        Ok(file_obj) => {
                            ...
                            match file_obj.read(&mut buffer) {
                                Ok(bytes_read) => {
                                    ...
                                    let expected =
                                        "Hello, this is a test write to FAT32 filesystem!";
                                    assert_eq!(
                                        content, expected,
                ...
                Err(e) => {
                    panic!(
                        "Written file (test_write.txt) not found, write operations failed: {:?}",
                        e
                    );
                }
            }
                        ...
                        Ok(entries) => {
                            ...
                            match fs.lookup(&top_dir_node, &String::from(sub_dir)) {
                                Ok(sub_dir_node) => {
                                    ...
                                    match fs
                                        .lookup(&sub_dir_node, &String::from(file_in_nested_dir))
                                    {
                                        Ok(file_node) => match fs.open(&file_node, 0) {
                                            Ok(file_obj) => {
                                                let mut buffer = vec![0u8; 64];
                                                match file_obj.read(&mut buffer) {
                                                    Ok(bytes_read) => {
                                                        let content = core::str::from_utf8(
                                                            &buffer[..bytes_read],
                                                        )
                                                        .unwrap_or("INVALID_UTF8");
                                                        early_println!(
                                                    ...
                                                    Err(e) => {
                                                        panic!(
                                                            "Failed to read file in nested directory: {:?}",
                                                            e
                                                        );
                                                    }
                                                }
                                            ...
                                            Err(e) => {
                                                panic!(
                                                    "Failed to open file in nested directory: {:?}",
                                                    e
                                                );
                                            }
                                        },
                                        Err(e) => {
                                            panic!(
                                                "Failed to lookup file in nested directory: {:?}",
                                                e
                                            );
                                        }
                                    }
        ...
        Err(e) => {
            panic!(
                "[Test] Warning: Failed to create FAT32 filesystem from virtio-blk device: {:?}",
                e
            );
        }
    }
...
fn test_fat32_delete_and_recreate() {
    ...
    for cycle in 1..=3 {
        ...
        // Verify content
        match fat32_fs.lookup(&root_node, &filename) {
            Ok(lookup_node) => match fat32_fs.open(&lookup_node, 0) {
                Ok(file_obj) => {
                    let mut buffer = vec![0u8; 64];
                    match file_obj.read(&mut buffer) {
                        Ok(bytes_read) => {
    ...
    match fat32_fs.readdir(&root_node) {
        Ok(entries) => {
            ...
            for dirname in &dirs {
                ...
                assert!(
                    found,
                    "Directory {} should exist in directory listing",
                    dirname
                );
            }
...
fn test_sfn_duplicate_handling() {
    ...
    for filename in &extension_variants {
        ...
        match fat32_fs.open(&file_node, 0x01) {
            ...
            Err(e) => {
                panic!(
                    "Failed to open extension variant {} for writing: {:?}",
                    filename, e
                );
            }
        }
    ...
    for (i, filename) in case_variants.iter().enumerate() {
        ...
        match fat32_fs.open(&file_node, 0x01) {
            ...
            Err(e) => {
                panic!(
                    "Failed to open case variant {} for writing: {:?}",
                    filename, e
                );
            }
        }
    ...
    for i in 1..=num_duplicates {
        ...
        match fat32_fs.open(&file_node, 0x01) {
            ...
            Err(e) => {
                panic!(
                    "Failed to open similar name file {} for writing: {:?}",
                    filename, e
                );
            }
        }
    ...
    for filename in &all_test_files {
        let found = entries
            .iter()
            .any(|e| e.name.to_uppercase() == filename.to_uppercase());
        assert!(found, "File {} should exist in directory listing", filename);
...
fn test_sfn_generation_edge_cases() {
    ...
    for (filename, description) in &edge_case_names {
        ...
        let file_node = fat32_fs
            ...
            .expect(&format!(
                "Failed to create edge case file {} ({})",
        ...
        match fat32_fs.open(&file_node, 0x01) {
            ...
            Err(e) => {
                panic!(
                    "Failed to open edge case file {} for writing: {:?}",
                    filename, e
                );
            }
        }
        ...
        match fat32_fs.lookup(&root_node, &filename.to_string()) {
            Ok(file_node) => {
                match fat32_fs.open(&file_node, 0x00) {
                    ...
                    Err(e) => {
                        panic!(
                            "Failed to open edge case file {} for reading: {:?}",
                            filename, e
                        );
                    }
                }
...
fn test_true_sfn_collision() {
    ...
    for (i, filename) in collision_filenames.iter().enumerate() {
        ...
        match fat32_fs.open(&file_node, 0x01) {
            ...
            Err(e) => {
                panic!(
                    "Failed to open collision file {} for writing: {:?}",
                    filename, e
                );
            }
        }
    ...
    for filename in &collision_filenames {
        ...
        assert!(
            found,
            "Collision file {} should exist in directory listing",
            filename
        );
                ...
                match fat32_fs.open(&file_node, 0x00) {
                    ...
                    Ok(file_obj) => {
                        ...
                        match file_obj.read(&mut buffer) {
                            ...
                            Err(e) => {
                                panic!(
                                    "Failed to read content of collision file {}: {:?}",
                                    filename, e
                                );
                            }
                        }
                    ...
                    Err(e) => {
                        panic!(
                            "Failed to open collision file {} for reading: {:?}",
                            filename, e
                        );
                    }
                }
    ...
    for i in 1..=num_files {
        ...
        match fat32_fs.open(&file_node, 0x01) {
            ...
            Err(e) => {
                panic!(
                    "Failed to open suffix test file {} for writing: {:?}",
                    filename, e
                );
            }
        }
        ...
        match fat32_fs.lookup(&root_node, &filename) {
            Ok(file_node) => {
                match fat32_fs.open(&file_node, 0x00) {
                    ...
                    Ok(file_obj) => {
                        ...
                        match file_obj.read(&mut buffer) {
                            Ok(bytes_read) => {
                                assert!(
                                    bytes_read > 0,
                                    "Suffix test file {} should have content",
                                    filename
                                );
                            }
                            Err(e) => {
                    ...
                    Err(e) => {
                        panic!(
                            "Failed to open suffix test file {} for reading: {:?}",
                            filename, e
                        );
                    }
                }
