...
impl VfsNode for OverlayNode {
    ...
    fn filesystem(&self) -> Option<alloc::sync::Weak<dyn FileSystemOperations>> {
        self.overlay_fs
            .read()
            .as_ref()
            .map(|fs| Arc::downgrade(fs) as alloc::sync::Weak<dyn FileSystemOperations>)
    }
...
impl OverlayFS {
    ...
    fn fs_from_mount(
        ...
        let filesystem = mount.root.node().filesystem().ok_or_else(|| {
            FileSystemError::new(
                FileSystemErrorKind::BrokenFileSystem,
                "Mount point has no filesystem",
            )
        })?;

        let fs_ops = filesystem.upgrade().ok_or_else(|| {
            FileSystemError::new(
                FileSystemErrorKind::BrokenFileSystem,
                "Filesystem operations are no longer available",
            )
        })?;
    ...
    fn read_link_for_path(&self, path: &str) -> Result<String, FileSystemError> {
        ...
        Err(FileSystemError::new(
            FileSystemErrorKind::NotFound,
            "Symbolic link not found in any layer",
        ))
    }
    ...
    fn resolve_in_layer(
        ...
        for part in parts {
            let current_fs = current_node
                .filesystem()
                .and_then(|w| w.upgrade())
                .ok_or_else(|| {
                    FileSystemError::new(
                        FileSystemErrorKind::NotSupported,
                        "Node has no filesystem",
                    )
                })?;
    ...
    fn is_whiteout(&self, path: &str) -> bool {
        if let Some((ref upper_fs, ref upper_node)) = self.upper {
            let whiteout_name = format!(".wh.{}", path.split('/').last().unwrap_or(path));
            ...
            self.resolve_in_layer(upper_fs, upper_node, &whiteout_path)
                .is_ok()
        } else {
            false
        }
    ...
    fn get_upper_layer(&self) -> Result<(Arc<MountPoint>, Arc<VfsEntry>), FileSystemError> {
        self.upper.as_ref().map(|fs| fs.clone()).ok_or_else(|| {
            FileSystemError::new(
                ...
                "Overlay is read-only (no upper layer)",
            )
        })
    }
            ...
            if let Ok(lower_node) = self.resolve_in_layer(lower_mount, lower_node, path) {
                ...
                match metadata.file_type {
                    ...
                    FileType::RegularFile => {
                        ...
                        if let Ok(source_file) = lower_fs.open(&lower_node, 0) {
                            // Read-only
    ...
    fn ensure_parent_dirs(&self, path: &str) -> Result<(), FileSystemError> {
        ...
        if self
            .resolve_in_layer(&upper.0, &upper.1, parent_path)
            .is_err()
        {
            ...
            let grandparent_node = self.resolve_in_layer(
                &upper.0,
                &upper.1,
    ...
    pub fn create_from_option_string(
        ...
        OverlayFS::new(upper, lower_layers, name).expect("Failed to create OverlayFS")
...
impl FileSystemOperations for OverlayFS {
    ...
    fn lookup(
        ...
        let overlay_parent = parent_node
            .as_any()
            .downcast_ref::<OverlayNode>()
            .ok_or_else(|| {
                FileSystemError::new(
                    FileSystemErrorKind::NotSupported,
                    "Invalid node type for OverlayFS",
                )
            })?;
    ...
    fn open(
        ...
        let overlay_node_ref = overlay_node
            .as_any()
            .downcast_ref::<OverlayNode>()
            .ok_or_else(|| {
                FileSystemError::new(
        ...
        if let Some((ref upper_mount, ref upper_node)) = self.upper {
            if let Ok(upper_node) =
                self.resolve_in_layer(upper_mount, upper_node, &overlay_node_ref.path)
            {
        ...
        if is_write_operation {
            return Err(FileSystemError::new(
                FileSystemErrorKind::PermissionDenied,
                "Cannot write to read-only overlay",
            ));
        }
        // Try lower layers for read operations
        for (lower_mount, lower_node) in &self.lower_layers {
            if let Ok(lower_node) =
                self.resolve_in_layer(lower_mount, lower_node, &overlay_node_ref.path)
            {
    ...
    fn remove(&self, parent_node: &Arc<dyn VfsNode>, name: &String) -> Result<(), FileSystemError> {
        ...
        if let Some((ref upper_mount, ref upper_entry)) = self.upper {
            if let Ok(upper_parent) =
                self.resolve_in_layer(upper_mount, upper_entry, &overlay_parent.path)
            {
                let fs = Self::fs_from_mount(upper_mount)?;
                if fs.remove(&upper_parent, name).is_ok() {
                    ...
                    for (lower_mount, lower_entry) in &self.lower_layers {
                        if self
                            .resolve_in_layer(lower_mount, lower_entry, &child_path)
                            .is_ok()
                        {
        ...
        for (lower_mount, lower_node) in &self.lower_layers {
            if self
                .resolve_in_layer(lower_mount, lower_node, &child_path)
                .is_ok()
            {
                self.create_whiteout(&child_path)?;
                return Ok(());
    ...
    fn readdir(
        ...
        let overlay_node = node.as_any().downcast_ref::<OverlayNode>().ok_or_else(|| {
            FileSystemError::new(
                FileSystemErrorKind::NotSupported,
                "Invalid node type for OverlayFS",
            )
        ...
        if let Some((ref upper_mount, ref upper_node)) = self.upper {
            if let Ok(upper_node) =
                self.resolve_in_layer(upper_mount, upper_node, &overlay_node.path)
            {
                let fs = upper_node
                    .filesystem()
                    .and_then(|w| w.upgrade())
        ...
        for (lower_mount, lower_node) in &self.lower_layers {
            if let Ok(lower_node) =
                self.resolve_in_layer(lower_mount, lower_node, &overlay_node.path)
            {
                let fs = lower_node
                    .filesystem()
                    .and_then(|w| w.upgrade())
...
impl OverlayDirectoryObject {
    ...
    fn collect_directory_entries(
        &self,
    ) -> Result<Vec<crate::fs::DirectoryEntryInternal>, FileSystemError> {
        ...
        if let Some((ref upper_mount, ref upper_node)) = self.overlay_fs.upper {
            if let Ok(upper_dir_node) =
                self.overlay_fs
                    .resolve_in_layer(upper_mount, upper_node, &self.path)
            {
        ...
        for (lower_mount, lower_node) in &self.overlay_fs.lower_layers {
            if let Ok(lower_dir_node) =
                self.overlay_fs
                    .resolve_in_layer(lower_mount, lower_node, &self.path)
            {
...
impl FileObject for OverlayDirectoryObject {
    ...
    fn metadata(&self) -> Result<crate::fs::FileMetadata, StreamError> {
        ...
        self.overlay_fs
            .get_metadata_for_path(&self.path)
            .map_err(StreamError::from)
    }
