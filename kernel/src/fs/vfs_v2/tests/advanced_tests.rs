...
fn test_basic_bind_mount() {
    ...
    vfs_manager
        .create_dir("/fs1")
        .expect("Failed to create /fs1");
    vfs_manager
        .create_dir("/fs2")
        .expect("Failed to create /fs2");

    vfs_manager
        .mount(tmpfs1.clone(), "/fs1", 0)
        .expect("Failed to mount /fs1");
    vfs_manager
        .mount(tmpfs2.clone(), "/fs2", 0)
        .expect("Failed to mount /fs2");

    // 2. Create a directory and a file in the source filesystem
    vfs_manager
        .create_dir("/fs1/dir_a")
        .expect("Failed to create /fs1/dir_a");
    vfs_manager
        .create_dir("/fs2/mount_point")
        .expect("Failed to create /fs2/mount_point");

    vfs_manager
        .create_file("/fs1/dir_a/file1", FileType::RegularFile)
        .expect("Failed to create file1");
    let file_obj = vfs_manager
        .open("/fs1/dir_a/file1", 0)
    ...
    vfs_manager
        .bind_mount("/fs1/dir_a", "/fs2/mount_point")
        .expect("Bind mount failed");

    // 4. Verification
    // Check if the file is accessible from the bind mount
    let bound_file_obj = vfs_manager
        .open("/fs2/mount_point/file1", 0x0000)
        .expect("Failed to open bound file"); // O_RDONLY
    let bound_file = bound_file_obj.as_file().unwrap();
    let mut read_buf = [0u8; 20];
    let bytes_read = bound_file
        .read(&mut read_buf)
        .expect("Failed to read from bound file");
    ...
    vfs_manager
        .create_file("/fs2/mount_point/file2", FileType::RegularFile)
        .expect("Failed to create file2 via bind mount");
    let new_file_obj = vfs_manager
        .open("/fs2/mount_point/file2", 0)
        .expect("Failed to open file2 via bind mount");
    ...
    let orig_file_obj = vfs_manager
        .open("/fs1/dir_a/file2", 0x0000)
        .expect("Failed to open original file2"); // O_RDONLY
    let orig_file = orig_file_obj.as_file().unwrap();
    let mut orig_read_buf = [0u8; 20];
    let orig_bytes_read = orig_file
        .read(&mut orig_read_buf)
        .expect("Failed to read from original file2");
...
fn test_nested_bind_mount() {
    ...
    vfs.create_file("/fs2/file2.txt", FileType::RegularFile)
        .unwrap();
    ...
    vfs.create_file("/fs1/dir_b/file_b", FileType::RegularFile)
        .unwrap();
    ...
    vfs.bind_mount("/fs1/dir_b", "/fs2/mount_a/nested_mount")
        .unwrap();
    ...
    let file_obj = vfs
        .open("/fs2/mount_a/nested_mount/../../file2.txt", 0)
        .unwrap();
    let file = file_obj.as_file().unwrap();
    assert!(file.metadata().is_ok());
}
...
fn test_unmount_bind_mount() {
    ...
    vfs.create_dir("/fs1/dir_a").unwrap();
    vfs.create_file("/fs1/dir_a/file1", FileType::RegularFile)
        .unwrap();
    vfs.create_dir("/fs2/mount_a").unwrap();
...
fn test_bind_mount_path_traversal() {
    ...
    vfs.create_file("/root_content.txt", FileType::RegularFile)
        .unwrap();
    ...
    vfs.create_file("/fs1/dir_a/dir_b/file_b", FileType::RegularFile)
        .unwrap();
    // Create a secret file in the bind mount source
    // This file should not be accessible through the bind mount
    // to prevent path traversal attacks.
    vfs.create_file("/fs1/dir_a/secret.txt", FileType::RegularFile)
        .unwrap();

    vfs.create_dir("/fs2/mount_point").unwrap();
    vfs.create_file("/fs2/file2", FileType::RegularFile)
        .unwrap();

    // 2. Bind mount /fs1/dir_a/dir_b onto /fs2/mount_point
    vfs.bind_mount("/fs1/dir_a/dir_b", "/fs2/mount_point")
        .unwrap();
    ...
    assert!(
        result.is_ok(),
        "Path traversal should allow access to /fs2/file2!"
    );
    ...
    assert!(
        result.is_err(),
        "Path traversal should not allow access to /fs1/dir_a/secret.txt!"
    );

    let result = vfs.metadata("/fs2/mount_point/../../root_content.txt");
    assert!(
        result.is_ok(),
        "Cannot access root_content.txt from bind mount!"
    );
}
