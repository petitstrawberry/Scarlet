...
fn test_symlink_cross_filesystem_file_operations() {
    ...
    let cpiofs =
        CpioFS::new("test_cpiofs".to_string(), &cpio_data).expect("Failed to create CpioFS");
    ...
    let _file_through_symlink = vfs
        .resolve_path("/symlink/bar")
        .expect("Failed to resolve /symlink/bar");

    // Read content from the source file through symlink using VFS open
    let source_content = {
        let file_obj = vfs
            .open("/symlink/bar", 0x01)
            .expect("Failed to open source file through symlink"); // Read mode
    ...
    vfs.create_file("/tmp/bar", FileType::RegularFile)
        .expect("Failed to create /tmp/bar");

    // Write content to destination file
    {
        let dest_file = vfs
            .open("/tmp/bar", 0x02)
            .expect("Failed to open destination file for writing"); // Write mode
    ...
    // Try to read back from destination file to verify it worked
    {
        let dest_file = vfs
            .open("/tmp/bar", 0x01)
            .expect("Failed to open destination file for reading"); // Read mode
        ...
        if let crate::object::KernelObject::File(dest_file_obj) = dest_file {
            ...
            assert_eq!(
                source_content, read_buffer,
                "Content mismatch between source and destination"
            );
        } else {
            panic!("Expected file object");
        }
...
fn copy_file_vfs(vfs: &VfsManager, src: &str, dest: &str) -> bool {
    ...
    // Read source file
    let src_file = match vfs.open(src, 0x01) {
        // Read mode
        Ok(file) => file,
        Err(_) => {
            return false;
    ...
    let dest_file = match vfs.open(dest, 0x02) {
        // Write mode
        Ok(file) => file,
        Err(_) => {
    ...
    let _verify_file = match vfs.open(dest, 0x01) {
        // Read mode
        Ok(file) => file,
        Err(_) => {
