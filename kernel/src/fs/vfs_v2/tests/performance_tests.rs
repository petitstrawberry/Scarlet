...
fn test_basic_performance() {
    ...
    if let Some(w) = weak_fs {
        ...
        assert!(
            upgraded.is_some(),
            "root_node.filesystem().upgrade() is None"
        );
    }
    ...
    for i in 0..file_count {
        ...
        manager
            .create_file(&filename, FileType::RegularFile)
            .unwrap();
    }
...
fn test_concurrent_operations() {
    // Create two separate VFS managers to simulate concurrent usage
...
fn test_deep_directory_performance() {
    ...
    manager
        .create_file(&deep_file, FileType::RegularFile)
        .unwrap();
...
fn test_mount() {
    ...
    manager
        .create_file("/mnt/fs1/test_file.txt", FileType::RegularFile)
        .unwrap();
...
fn test_nested_mounts() {
    ...
    manager
        .create_file("/mnt/fs1/test_file1.txt", FileType::RegularFile)
        .unwrap();
    ...
    manager
        .create_file("/mnt/fs1/dir/fs2/test_file2.txt", FileType::RegularFile)
        .unwrap();
    ...
    let metadata = manager
        .metadata("/mnt/fs1/dir/fs2/../../test_file1.txt")
        .unwrap();
    assert_eq!(metadata.file_type, FileType::RegularFile);
}
