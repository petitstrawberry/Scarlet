...
pub enum ServEvent<'g, 'a> {
    ...
    /// Client changed the terminal dimensions for a session channel.
    SessionWindowChange(ServWindowChange<'g, 'a>),
...
impl<'g, 'a> ServPtyRequest<'g, 'a> {
    ...
    /// Return the requested terminal type, dimensions, and terminal modes.
    ///
    /// # Returns
    ///
    /// An owned [`Pty`] description, or an SSH packet decoding error.
...
/// A terminal window-size change for a server session channel.
pub struct ServWindowChange<'g, 'a> {
    runner: &'g mut Runner<'a, Server>,
    num: ChanNum,
}

impl ServWindowChange<'_, '_> {
    /// Return the associated channel number.
    ///
    /// # Returns
    ///
    /// The local channel number corresponding to an accepted session.
    pub fn channel(&self) -> ChanNum {
        self.num
    }

    /// Return the requested character and pixel dimensions.
    ///
    /// # Returns
    ///
    /// `(columns, rows, pixel_width, pixel_height)`, or an SSH packet decoding
    /// error.
    pub fn dimensions(&self) -> Result<(u32, u32, u32, u32)> {
        let change = self.runner.fetch_window_change()?;
        Ok((change.cols, change.rows, change.width, change.height))
    }
}
...
impl ServEventId {
    pub fn event<'g, 'a>(
        ...
        match self {
            ...
            Self::WindowChange { num } => {
                debug_assert!(matches!(p, Some(Packet::ChannelRequest(_))));
                Ok(ServEvent::SessionWindowChange(ServWindowChange {
                    runner,
                    num,
                }))
            }
    ...
    pub(crate) fn needs_resume(&self) -> bool {
        match self {
            Self::Defunct | Self::WindowChange { .. } => false,
            Self::Hostkeys
            | Self::FirstAuth
            | Self::PasswordAuth
            | Self::PubkeyAuth { .. }
