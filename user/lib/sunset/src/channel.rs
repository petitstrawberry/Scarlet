...
impl Channels {
    ...
    pub(crate) fn send_exit_status(
        ...
        self.get(num)?.send_exit_status(status, s)
    }
    ...
    pub(crate) fn send_close(&mut self, num: ChanNum, s: &mut TrafSend) -> Result<()> {
        self.get_mut(num)?.send_close(s)
    }
    ...
    pub(crate) fn fetch_servpty(&self, p: &Packet<'_>) -> Result<Pty> {
        match p {
            Packet::ChannelRequest(ChannelRequest {
                req: ChannelReqType::Pty(request),
                ..
            }) => request.try_into(),
            _ => Err(Error::bug()),
        }
    ...
    pub(crate) fn fetch_window_change(&self, p: &Packet<'_>) -> Result<packets::WinChange> {
        match p {
            Packet::ChannelRequest(ChannelRequest {
                req: ChannelReqType::WinChange(request),
                ..
            }) => Ok(request.clone()),
            _ => Err(Error::bug()),
        }
...
impl Channel {
    ...
    fn dispatch_server_request(
        ...
        match &p.req {
            ...
            ChannelReqType::WinChange(_) => {
                Ok(DispatchEvent::ServEvent(ServEventId::WindowChange { num }))
            }
            _ => {
    ...
    fn send_exit_status(&self, status: u32, s: &mut TrafSend) -> Result<()> {
        let packet = ChannelRequest {
            num: self.send_num()?,
            want_reply: false,
    ...
    fn send_close(&mut self, s: &mut TrafSend) -> Result<()> {
        if !self.sent_close {
