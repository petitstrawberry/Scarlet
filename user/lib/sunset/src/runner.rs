...
impl<'a> Runner<'a, server::Server> {
    ...
    /// Send the remote process exit status on a session channel.
    ///
    /// # Arguments
    ///
    /// * `channel` - Open server session channel.
    /// * `status` - Unsigned process exit status defined by RFC 4254.
    ///
    /// # Returns
    ///
    /// `Ok(())` when the request was queued, or an SSH transport error.
    pub fn send_channel_exit_status(
        &mut self,
        channel: &ChanHandle,
        status: u32,
    ) -> Result<()> {
        let mut sender = self.traf_out.sender(&mut self.keys);
        self.conn
            .channels
            .send_exit_status(channel.0, status, &mut sender)?;
        self.wake();
        Ok(())
    }

    /// Send EOF in the server-to-client direction of a channel.
    ///
    ...
    /// # Returns
    ///
    /// `Ok(())` when EOF was queued or had already been sent, otherwise an
    /// SSH transport error.
    pub fn send_channel_eof(&mut self, channel: &ChanHandle) -> Result<()> {
        let mut sender = self.traf_out.sender(&mut self.keys);
        self.conn.channels.send_eof(channel.0, &mut sender)?;
        self.wake();
        Ok(())
    }

    /// Send a close message for a server channel.
    ///
    /// Calling this method more than once is harmless. Applications normally
    /// send EOF before closing a session channel.
    ///
    /// # Arguments
    ///
    /// * `channel` - Open server session channel.
    ...
    /// # Returns
    ///
    /// `Ok(())` when close was queued or had already been sent, otherwise an
    /// SSH transport error.
    pub fn send_channel_close(&mut self, channel: &ChanHandle) -> Result<()> {
        let mut sender = self.traf_out.sender(&mut self.keys);
        self.conn.channels.send_close(channel.0, &mut sender)?;
        self.wake();
        Ok(())
    }
}
...
impl<'a, CS: CliServ> Runner<'a, CS> {
    ...
    fn check_chanreq(prev_event: &DispatchEvent) {
        debug_assert!(matches!(
            ...
            DispatchEvent::ServEvent(ServEventId::SessionShell { .. })
                ...
                | DispatchEvent::ServEvent(ServEventId::WindowChange { .. })
        ));
    }
    ...
    pub(crate) fn fetch_window_change(&self) -> Result<packets::WinChange> {
        Self::check_chanreq(&self.resume_event);
        let (payload, _seq) = self.traf_in.payload().trap()?;
        let p = self.conn.packet(payload)?;
