...
impl View for VStack {
    fn layout(&mut self, available: Size) -> Size {
        ...
        if !self.children.is_empty() {
            total_height += self.spacing * (self.children.len() as u32 - 1);
        }
...
impl View for HStack {
    fn layout(&mut self, available: Size) -> Size {
        ...
        // Add spacing
        if !self.children.is_empty() {
            total_width += self.spacing * (self.children.len() as u32 - 1);
        }
...
impl View for Center {
    fn layout(&mut self, available: Size) -> Size {
        ...
        available // Take all available space
    }
    ...
    fn children(&self) -> Vec<(&dyn View, Rect)> {
        let x = (0i32 - self.cached_size.width as i32) / 2; // Will be adjusted by parent
        let y = (0i32 - self.cached_size.height as i32) / 2;
        let child_frame = Rect::new(x, y, self.cached_size.width, self.cached_size.height);
        let mut v = Vec::new();
        v.push((self.child.as_ref() as &dyn View, child_frame));
        v
    ...
    fn children_mut(&mut self) -> Vec<(&mut dyn View, Rect)> {
        let x = (0i32 - self.cached_size.width as i32) / 2;
        let y = (0i32 - self.cached_size.height as i32) / 2;
        let child_frame = Rect::new(x, y, self.cached_size.width, self.cached_size.height);
        let mut v = Vec::new();
        v.push((self.child.as_mut() as &mut dyn View, child_frame));
