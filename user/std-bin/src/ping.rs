            ...
            if let Ok(stream) = socket.as_stream() {
                match stream.read(&mut buf) {
                    Ok(read) if read == payload.len() && &buf[..read] == payload => {
                        ...
                        println!(
                            "[ping] reply {read} bytes, seq={sequence}, rtt {rtt_ms}.{rtt_us:03} ms"
                        );
...
fn print_summary(
    ...
    println!(
        "{transmitted} packets transmitted, {received} packets received, {loss_percent}% packet loss"
    );
    ...
    if received > 0 {
        ...
        println!(
            "rtt min/avg/max = {min_ms}.{min_us:03}/{average_ms}.{average_us:03}/{max_ms}.{max_us:03} ms"
        );
    }
}
