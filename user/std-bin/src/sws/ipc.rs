...
fn poll_atomic_handle_frame(
    ...
    let required_len = match socket.recv_handle_and_data(&mut probe) {
        Err(scarlet_os::socket::SocketError::ReceiveBufferTooSmall { required_len }) => {
            required_len
        }
        Err(scarlet_os::socket::SocketError::WouldBlock) => return Ok(None),
...
pub fn set_compositor_wake_handle(write_handle: Handle) {
    let mut wake = COMPOSITOR_WAKE_WRITE.lock().expect("SWS mutex poisoned");
...
pub fn wake_compositor() {
    ...
    let wake = COMPOSITOR_WAKE_WRITE.lock().expect("SWS mutex poisoned");
...
fn wake_client(client_id: usize) {
    let mut wakes = CLIENT_WAKES.lock().expect("SWS mutex poisoned");
...
pub fn push_ipc_event(event: IpcEvent) {
    let mut queue = EVENT_QUEUE.lock().expect("SWS mutex poisoned");
...
pub fn has_pending_ipc_events() -> bool {
    !EVENT_QUEUE.lock().expect("SWS mutex poisoned").is_empty()
}
...
fn register_window(window_id: u32, client_id: usize) {
    WINDOW_OWNERS
        .lock()
        .expect("SWS mutex poisoned")
        .insert(window_id, client_id);

    {
        let mut pending = PENDING_INPUT_EVENTS.lock().expect("SWS mutex poisoned");
...
pub fn send_extension_input_event(
    ...
    let mut pending = PENDING_EXTENSION_INPUT_EVENTS
        .lock()
        .expect("SWS mutex poisoned");
...
pub fn send_pointer_lock_input_event(
    ...
    let mut normal = PENDING_INPUT_EVENTS.lock().expect("SWS mutex poisoned");
    let mut extension = PENDING_EXTENSION_INPUT_EVENTS
        .lock()
        .expect("SWS mutex poisoned");
...
fn activate_text_input_for_window(window_id: u32) {
    let next_context = {
        let contexts = TEXT_INPUT_CONTEXTS.lock().expect("SWS mutex poisoned");
        contexts
            .values()
    ...
    let old_context_id = {
        let mut active = ACTIVE_TEXT_INPUT_CONTEXT
            .lock()
            .expect("SWS mutex poisoned");
...
pub fn set_focused_window(window_id: u32) {
    {
        let mut focused = FOCUSED_WINDOW_ID.lock().expect("SWS mutex poisoned");
...
fn cleanup_text_input_contexts_for_client(client_id: usize) {
    let active_id = *ACTIVE_TEXT_INPUT_CONTEXT
        .lock()
        .expect("SWS mutex poisoned");
...
fn set_text_input_enabled(client_id: usize, context_id: u32, enabled: bool) {
    ...
    let focused_window = *FOCUSED_WINDOW_ID.lock().expect("SWS mutex poisoned");
...
pub fn set_preferred_input_method(name: Option<String>) {
    *PREFERRED_IME_NAME.lock().expect("SWS mutex poisoned") = name;
}
...
fn register_input_method(client_id: usize, name: &[u8], capabilities: u32) -> InputMethodService {
    ...
    INPUT_METHODS
        .lock()
        .expect("SWS mutex poisoned")
        .insert(ime_id, service.clone());

    let preferred_name = PREFERRED_IME_NAME
        .lock()
        .expect("SWS mutex poisoned")
        .clone();
    let should_activate = ACTIVE_IME_ID.lock().expect("SWS mutex poisoned").is_none()
        || preferred_name.as_deref() == Some(service.name.as_str());
    if should_activate {
...
fn activate_input_method(ime_id: u32, persist_selection: bool) -> bool {
    let Some(service) = INPUT_METHODS
        .lock()
        .expect("SWS mutex poisoned")
        .get(&ime_id)
        .cloned()
    ...
    let changed = {
        let mut active = ACTIVE_IME_ID.lock().expect("SWS mutex poisoned");
    ...
    if changed {
        ...
        if let Some(context_id) = *ACTIVE_TEXT_INPUT_CONTEXT
            .lock()
            .expect("SWS mutex poisoned")
    ...
    if persist_selection {
        *PREFERRED_IME_NAME.lock().expect("SWS mutex poisoned") = Some(service.name.clone());
...
fn input_methods_payload() -> Vec<u8> {
    let active_ime_id = *ACTIVE_IME_ID.lock().expect("SWS mutex poisoned");
    let methods = INPUT_METHODS.lock().expect("SWS mutex poisoned");
...
fn drain_pending_ime_keys(context_id: Option<u32>) -> Vec<PendingImeKey> {
    let mut pending = PENDING_IME_KEYS.lock().expect("SWS mutex poisoned");
...
fn set_ime_keyboard_grabbed(client_id: usize, context_id: u32, grabbed: bool) {
    ...
    let active_context_id = *ACTIVE_TEXT_INPUT_CONTEXT
        .lock()
        .expect("SWS mutex poisoned");
    let mut contexts = TEXT_INPUT_CONTEXTS.lock().expect("SWS mutex poisoned");
    let Some(context) = contexts.get_mut(&context_id) else {
...
pub fn send_input_method_trigger(window_id: u32, time: u64, code: u16) -> bool {
    let Some(context_id) = *ACTIVE_TEXT_INPUT_CONTEXT
        .lock()
        .expect("SWS mutex poisoned")
    else {
        return false;
...
fn cleanup_input_methods_for_client(client_id: usize) {
    let removed: Vec<u32> = {
        let mut methods = INPUT_METHODS.lock().expect("SWS mutex poisoned");
    ...
    if active_was_removed {
        release_pending_ime_keys(None);
        clear_keyboard_grabs();
        *ACTIVE_IME_ID.lock().expect("SWS mutex poisoned") = None;
...
pub fn send_key_to_input_method(
    ...
    PENDING_IME_KEYS.lock().expect("SWS mutex poisoned").insert(
        key_serial,
        PendingImeKey {
...
fn client_is_active_input_method(client_id: usize) -> bool {
    let Some(ime_id) = *ACTIVE_IME_ID.lock().expect("SWS mutex poisoned") else {
        return false;
    };
    INPUT_METHODS
        .lock()
...
pub fn get_app_session_info(window_id: u32) -> (String, String) {
    let sessions = APP_SESSIONS.lock().expect("SWS mutex poisoned");
...
fn accept_thread_main(server_socket: Socket) {
    ...
    loop {
        ...
        match server_socket.accept() {
            Ok(client_socket) => {
                ...
                let client_wake = match scarlet_os::ipc::pipe() {
                    Ok((wake_read, wake_write)) => {
                        CLIENT_WAKES.lock().expect("SWS mutex poisoned").insert(
                            client_id,
                            ClientWake {
            ...
            Err(scarlet_os::socket::SocketError::WouldBlock) => {
                            ...
                            Ok(mapper) => {
                                match mapper.mmap(
                                    ...
                                    Ok(addr) => {
                                        ...
                                        // Sample the first bytes only when the mapping is large
                                        // enough to make every diagnostic read valid.
                                        if buffer_size >= 16 {
                                            // SAFETY: `addr..addr + 16` lies inside the live
                                            // mapping after the size check above.
                                            unsafe {
