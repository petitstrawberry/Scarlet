...
pub fn run() -> i32 {
    ...
    let mut config = MachineConfig::qemu_virt_aarch64();
    ...
    let gic = Arc::new(GicDevice::new(0x0800_0000, 0x080A_0000));
    println!("[ushv] Registered GIC at 0x08000000");

    let uart = Arc::new(Ns16550a::with_irq(0x0900_0000, 33));
    let vcpu_irq = IrqLine::new(Arc::new(VcpuIrqSink::new(vcpu.handle())));
    uart.set_irq_out(vcpu_irq);
    println!("[ushv] Registered UART at 0x09000000");

    let uart_for_thread = Arc::clone(&uart);

    machine.register(gic);
    ...
    println!("[ushv] Timer thread initialized");
    ...
    if vcpu.set_reg(reg::X0, guest_dtb_addr).is_err() {
        println!("[ushv] Failed to set x0 (dtb address)");
        return 1;
    }
    println!("[ushv] Set boot register: x0={:#x}", guest_dtb_addr);

    let mut firmware = PsciFirmware::new();
