...
pub fn run() -> i32 {
    ...
    // SAFETY: The guest range remains allocated until process teardown; loading
    // is finished and host accesses are coordinated with vCPU execution.
    ...
    loop {
        ...
        match exit.reason {
            VcpuExitReason::MmioRead => {
                ...
                if exit.mmio.reg != 0 && vcpu.set_reg(exit.mmio.reg, masked).is_err() {
                    println!(
...
fn allocate_guest_memory(size: usize) -> usize {
    ...
    // SAFETY: This allocates a fresh non-fixed guest range without replacing
    // live memory; the VM retains it until process teardown.
