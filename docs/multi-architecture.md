...
- **x86_64 (AMD64)** - Basic support available
...
# x86_64
ARCH=x86_64 cargo make build
ARCH=x86_64 cargo make test
```
...
# x86_64 (uses buildroot-x86_64 config)
ARCH=x86_64 bash tools/linux/build_buildroot.sh
```
...
# Deploy x86_64 rootfs
ARCH=x86_64 bash tools/linux/deploy_rootfs.sh
```
...
```bash
cargo make debug-x86_64      # Debug mode
cargo make debug-test-x86_64 # Test mode
```

Or manually:
```bash
bash kernel/tools/run_x86_64.sh
```
...
- `kernel/tools/test_x86_64.sh` - x86_64 test runner
...
- `kernel/lds/x86_64_qemu_virt.ld`

**User space:**
- `user/lds/user.ld` (RISC-V)
- `user/lds/user_aarch64.ld` (AArch64)
- `user/lds/user_x86_64.ld` (x86_64)
...
- x86_64 uses ACPI (generated by QEMU)
...
- Register naming: `rax`, `rbx`, `rcx`, `rdx`, `rsi`, `rdi`, `rsp`, `r8-r15`, `rip`
- Calling convention (System V AMD64 ABI): `rdi`, `rsi`, `rdx`, `rcx`, `r8`, `r9` (args), `rax` (return)
- Privilege levels: Ring 0 (kernel), Ring 3 (user)
- Page table: 4KB pages, 4-level paging (PML4, PDP, PD, PT)
- Interrupt handling: Local APIC, I/O APIC, 8259 PIC (legacy)
